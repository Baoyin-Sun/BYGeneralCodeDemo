...
#import "UIImage+BYCustom.h"

@implementation UIImage (BYCustom)
...
+ (UIImage *)resizeWithScale:(NSString *)imageName Wscale:(CGFloat)w Hscale:(CGFloat)h {
    UIImage * image = [UIImage imageNamed: imageName];
    int W = image.size.width * w;
    int H = image.size.height * h;
    return [image resizableImageWithCapInsets: UIEdgeInsetsMake(H, W, image.size.height - H - 1 , image.size.width - W - 1)];
}

+ (UIImage *)headerPlaceholder {
    return [UIImage imageNamed:@"ic_personAvatar"];
}
...
- (UIImage *)drawCircleImage {
    return  [self drawCircleImageWithRadius:self.size.width*0.5];
}
